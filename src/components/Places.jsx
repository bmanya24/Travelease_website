...
const Place = () => {
  ...
  const debouncedSearch = _debounce((value) => {
    ...
    axios
      .get(`http://localhost:5000/destinations?search=${value}`) // Updated URL
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/destinations") // Updated URL
      .then((response) => {
        setPlacesData(response.data);
      })
      .catch((error) => {
  ...
  const handleBook = (place) => {
    const userData = JSON.stringify({ ...place, location: selectedLocation }); // Include selected location
